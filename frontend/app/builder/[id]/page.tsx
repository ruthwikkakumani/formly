...
const types = [["short_text","Short text"],["long_text","Long text"],["multiple_choice","Multiple choice"],["dropdown","Dropdown"],["email","Email"],["number","Number"],["yes_no","Yes / No"],["rating","Rating"],["file_upload","File upload"]];
  ...
  return <main className="builder">
    ...
    {tab === "Build" && <div className="buildbody">
      ...
      <section className="editor"><div className="qnumber">QUESTION {selected + 1} OF {form.questions.length}</div>
        ...
        {["multiple_choice", "dropdown", "yes_no"].includes(question.type) && <div className="logic"><b>Logic jump (optional)</b><select value={question.logic?.option || ""} onChange={e => changeQuestion("logic", { ...(question.logic || {}), option: e.target.value })}><option value="">No logic jump</option>{(question.type === "yes_no" ? ["Yes", "No"] : question.options).map((option: string) => <option value={option} key={option}>When answer is {option}</option>)}</select>{question.logic?.option && <select value={question.logic?.target ?? ""} onChange={e => changeQuestion("logic", { ...question.logic, target: e.target.value })}><option value="">Then continue normally</option>{form.questions.map((item: any, index: number) => index > selected && <option value={index} key={index}>Jump to {index + 1}. {item.title}</option>)}</select>}</div>}
...
function Settings({ form, change, save }: any) {
  ...
  const update = (key: string, value: any) => change("theme", { ...theme, [key]: value });
  return <section className="settings"><h2>Settings</h2><p>Make this form feel like yours.</p><div className="settingsgrid"><article><h3>Theme</h3><label>Background <input type="color" value={theme.background || "#f7f7f4"} onChange={e => update("background", e.target.value)} /></label><label>Text color <input type="color" value={theme.color || "#262627"} onChange={e => update("color", e.target.value)} /></label><label className="toggle">Dark response experience <input type="checkbox" checked={Boolean(theme.darkMode)} onChange={e => update("darkMode", e.target.checked)} /></label><button className="primary" onClick={save}>Save theme</button></article><article><h3>Thank-you screen</h3><textarea value={theme.thankYou || "Your response has been submitted."} onChange={e => update("thankYou", e.target.value)} /><button className="primary" onClick={save}>Save message</button></article></div><div className="coming"><article><b>Integrations & webhooks</b><span>Coming soon</span></article><article><b>Team collaboration</b><span>Coming soon</span></article><article><b>Payments</b><span>Coming soon</span></article></div></section>;
}
